...
def create_db(db_name):
    db = mysql.connector.connect(
        host=_env("DB_HOST"),
        port=int(_env("DB_PORT")), # Обов'язково int(), бо порт має бути числом
        user=_env("DB_USER"),
        password=_env("DB_PASSWORD"),
        auth_plugin=_env("DB_AUTH_PLUGIN")
    )
    cursor = db.cursor()
...
def parse_copart_lot(lot_obj, file_path, cursor, db, db_name, table_name): #extracts data from single lot and adds it into db
    ...
    # copart to final db structure dictionary:
    fields_dict = {
        'dynamicLotDetails.buyTodayBid' : {
            'table_name': 'fact_listings',
            'column_name': 'buy_now_bid'
        },
        'dynamicLotDetails.currentBid' : {
            'table_name': 'fact_listings',
            'column_name': 'current_bid'
        },
        'mkn': { # should be fulled by photo to vin transformer
            'table_name': 'ref_makes',
            'column_name': 'name'
        },
        'dynamicLotDetails.lotSold': 'enabled', # if lotSold false set enabled to true
        'vehTypDesc': {
            'table_name': 'dim_vehicle_types', # automobile, bus, etc.
            'column_name': 'name'
        },
        'carFaxReportAvailable': {
            'table_name': 'dim_vehicles',
            'column_name': 'carFax_report_available'
        },
        'lotNumberStr': 'lot_number',
        'mmod': { # should be fulled by photo to vin transformer
            'table_name': 'ref_models',
            'column_name': 'name'
        },
        'mtrim': { # should be fulled by photo to vin transformer
            'table_name': 'ref_trims',
            'column_name': 'name'
        },
        'lcy': {
            'table_name': 'new_ref_years', #should be created and used for manheim also
            'column_name': 'name' # ? the column name
        },
        'fv': {
            'table_name': 'dim_vehicles', # should be fulled by photo to vin transformer
            'column_name': 'vin'
        },
        'la': {
            'table_name': 'fact_listings',
            'column_name': 'estimated_real_price'
        },
        'egn': {
            'table_name': 'ref_displacements', #should be formated (delete all after letter L)
            'column_name': 'name'
        },
        'cy': {
            'table_name': 'ref_cylinders',
            'column_name': 'name'
        },
        'yn': { # it's location, not always, it's the same as syn. But When you choose https://www.copart.com/salesListResult u'll find only by syn'
            'table_name': 'dim_auctions',
            'column_name': 'location',
            'name': 'Copart'
        },
        # треба буде не упустити момент конвертації валют
        'lad': {  # + at + ianaTimeZone
            'table_name': 'new_dates',
            'column_name': 'sale_date_time'
        },
        'dd': { # primary damage
            'table_name': 'lot_damages',
            'column_name': 'damage_description', #
            'severity_order': '1'
        },
        'locCountry':{ # there also can be added the city locCity and state locState but I think there is no need. It will raise the question about creating a new table for that.
            'table_name': 'dim_auctions',
            'column_name': 'new_country'
        },
        'tmtp':{
            'table_name': 'ref_transmissions', # should be assigned with the existing ones
            'column_name': 'name'
        },
        'sdd': { # secondary damage
            'table_name': 'lot_damages',
            'column_name': 'damage_description', #
            'severity_order': '2'
        },
        'lcd': {
            'table_name': 'dim_highlights',
            'column_name': 'description' # should be assigned with the existing ones. On copart: RUNS AND DRIVES on manheim: Run & Drive
        },
        'clr': {
            'table_name': 'dim_colors',
            'column_name': 'name'
        },
        'ft': {
            'table_name': 'ref_fuels',
            'column_name': 'name' # should be assigned
        },
        'hk': {
            'table_name': 'dim_keys',
        ...
        'drv': {
            'table_name': 'ref_drives',
            'column_name': 'name' # should be assigned
        },
        'scn': { # ensurance
            'table_name': 'new_ensurances',
            'column_name': 'name'
        },
        # brand  it can be copart, unknown, or crashedtoys . They are probably working with crashedtoys, cause it's smth like auction for motorcycles
        'ldu': {
            'table_name': 'fact_listings', # should be written by https://www.copart.com/lot/lotNumberStr/ldu
            'column_name': 'lot_url'
        },
        'ord': {
            'table_name': 'dim_odometer_statuses',
            'column_name': 'status_name' # should be assigned
        },
        'ts': { # може правильніше сюди буде записати td
            'table_name': 'dim_doc_statuses',
            'column_name': 'status_name'
        },
        'bstl': {
            'table_name': 'ref_body_types',
            'column_name': 'name' # should be assigned
        },
        'ext_color': {
            'table_name': 'new_ext_int_colors',
            'column_name': 'ext_color'
        },
        'int_color': {
            'table_name': 'new_ext_int_colors',
            'column_name': 'int_color'
        },
        'cr_damages': {
            'table_name': 'new_damages_details',
            'column_name': 'data' # там json з силками на фотки і з параметрами з інфою всякою
        },
        'orr': {
            'table_name': 'fact_listings',
            'column_name': 'odometer_km' # should be converted into km, all the values are in miles, there is no column with value km
        },
        '': {
            'table_name': '',
            'column_name': ''
    ...
    fields_usefull_but_not_regulated = {
        'lmg', # smth about modifications                                                SELECT * FROM manheim.copart_lots_flat where `mmod` != `lmg`;
        'lm' # smth about modifications, usually a sum of mmod and mtrim, but not always SELECT * FROM manheim.copart_lots_flat where `mmod` != `lmg`;
    }

    fields_idk = {
        'siteCodes', #probably copart us, copart canada, etc. select siteCodes from manheim.copart_lots_flat where siteCodes not in ('["CPRTUS"]', '["CPRTCA", "CPRTUS"]');
        'lotPlugAcv',
        'pwlot', # i couldn't find a least a single lot where this field = 1 and this lot still exists
        'sellerEligibleVVV', # it's smth about the seller so probably useless
        'syn',# наименование продажи. На https://www.copart.com/salesListResult ти знайдеш саме по ньому, але це назва продажу, а не локація. А от локація це yn
        #   {
        #     'table_name': '',
        #     'column_name': ''
        # },
        #
        'lic', #якісь там внутрішні відмітки і перевірки
        'dtc',
        # 'locCity':{
        #     'table_name': 'new_country_city', # should be created
        #     'column_name': 'city'
        # },
        'locState',
        'smd.instagramUrl', # there is only one link, no others  https://www.instagram.com/farmerssalvage/?hl=en
        'smd.facebookUrl', # there is only one link, no others  https://www.facebook.com/FarmersInsuranceSalvageAndRecycling/
        'lipn', # vehicle plate number
        'td', # щось там по сертифікатам, схоже на ts , але інше
        'lfd', # оці хот айтемс, но лайсенс реквайред і так далі, оці значки мальовані круглі
        'sln',
        '',
        'hb' # 'new_current_bid', #the same as current bid that is already present
        '',
